...
v_check <- (v_filter)
spec_sum <- function( selected_year, selected_state) {colSums(gundeaths_cut[,3:4])}
View(gundeaths_cut)
...
library(caret)
proc_lawprov <- preProcess(as.data.frame(law_provision_norm$lawtotal), method = c("range"))
